...
-- daftar nama / kata kunci objek yang TIDAK boleh di-hide
local VisibleExceptions = {
    "banana", "Banana", "Cash", "Inventory", "Hotbar"
}

-- fungsi pengecekan exception
local function isException(obj)
    for _, keyword in ipairs(VisibleExceptions) do
...
local function applySafeOptimizations(enable)
    ...
    -- Hide objek kecuali exception
    for _, obj in ipairs(workspace:GetDescendants()) do
        pcall(function()
            if isException(obj) then return end  -- <=== pengecualian disini
    ...
    -- Sembunyikan GUI (kecuali inventory/banana)
    if LocalPlayer:FindFirstChild("PlayerGui") then
        for _, gui in ipairs(LocalPlayer.PlayerGui:GetChildren()) do
            pcall(function()
                if isException(gui) then return end  -- <=== pengecualian GUI
...
workspace.DescendantAdded:Connect(function(obj)
    pcall(function()
        if isException(obj) then return end  -- <=== pengecualian real-time
        if obj:IsA("BasePart") then
            if typeof(obj.LocalTransparencyModifier) == "number" then
...
-- GUI overlay tetap sama (tidak diubah)
